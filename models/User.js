...
var deps = {
	...
	qq:{'services.qq.isConfigured':true},
...
User.add({
	...
	isAdmin: { type: Boolean, label: '是否为管理员', index: true },
...
User.schema.methods.resetPassword = function(callback) {
	...
	user.save(function(err) {
		...
		new keystone.Email('forgotten-password').send({
			...
			subject: 'Reset your PerfectInfo Password',
			to: user.email,
			from: {
				name: 'PerfectInfo',
				email: 'jinbinli@itsi.com.cn'
			}
		}, callback);
...
User.relationship({ ref: 'worksheet', path: 'author' });
