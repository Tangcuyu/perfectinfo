...
block content
	form(method='post', enctype='multipart/form-data').item-details
		...
		.toolbar.toolbar-fixed
			if !list.get('noedit')
				button(type='submit').btn.btn-default.btn-save 保存
				a(href='/keystone/' + list.path + '/' + item.id, data-confirm='你确认要重置所有的信息吗？').btn.btn-link.btn-cancel 重置
			if !list.get('nodelete')  && item.id !== user.id
				a(href='/keystone/' + list.path + '?delete=' + item.id + csrf_query, data-confirm='你确认要删除此' + list.singular.toLowerCase() + '?').btn.btn-link.btn-cancel 删除 #{list.singular.toLowerCase()}
