...
block content
		...
		.create-item
			if list.get('autocreate')
				...
				form(method='post', action='/keystone/' + list.path)
					...
					.toolbar
						.toolbar-create
							button(type='submit').btn.btn-default.btn-create 创建
							button(type='button').btn.btn-link.btn-cancel.btn-cancel-create-item 取消
						.toolbar-default
	...
	.page-header.list-header(class=!items.total && !search && !xFilters ? 'empty-list' : '')
		h1
			if items.total
				span.items-total= '总计：' + utils.plural(items.total, '* ' + list.singular, '* ' + list.plural) + ' '
			else
				span.items-total 没有找到 #{list.plural.toLowerCase()} 
					= search || xFilters ? ' ' : '.'
		...
		form#list-filters
				...
				.list-toolbar
						...
						.form-inline
							.dropdown-inline.list-filter-dropdown.js-recent-searches.hidden.mr-5
								button(type='button', data-toggle='dropdown', title='Recent Filters').btn.btn-default.dropdown-toggle
									span.sr-only 最近使用的过滤器
									span.ion-clock
									|  
									span.caret
								ul.dropdown-menu
									li.dropdown-header 最近使用的过滤器
							.form-inline.searchbox-form.mr-5
								.searchbox-field
									input(type='search', name='search', placeholder='搜索 #{list.plural.toLowerCase()}', value=search).form-control.js-search-list.searchbox-input
									if search
								...
								.searchbox-button: button(type='submit').btn.btn-default.btn-search.searchbox-submit
									...
									span.hidden-xs 搜索
							//- .search-field
								input(type='text', name='search', placeholder='搜索 ' + list.plural.toLowerCase(), value=search).form-control
					...
					if items.total
						.pull-right.hidden-xs
							.pull-right
								a(href=download_link, data-confirm="Download a .csv of #{utils.plural(items.total, '* ' + list.singular, '* ' + list.plural).toLowerCase()}?").btn.btn-default 下载列表
			...
			.list-filters
				each field in list.fields
					if !field.nofilter && !field.hidden
						.filter(data-path=field.path, data-type=field.type, class=filters[field.path] ? 'active' : '')
							.filter-label
								a(href=js, title='删除此过滤器', data-toggle='tooltip', data-placement='right', data-container='body').btn-companion-xs.clear-filter &times;
							...
							.filter-options
								...
								if field.type == 'text' || field.type == 'textarea' || field.type == 'html' || field.type == 'email' || field.type == 'url' || field.type == 'key'
									...
									button(type='button', data-toggle='button', class=ops.inverse ? 'active' : '', data-opt='inv', data-value='true').btn.btn-xs.btn-default 相反
				...
				.list-filters-action
					button(type='submit').btn.btn-default.btn-filter 搜索
					a(href='/keystone/#{list.path}').btn.btn-link.btn-cancel 清除过滤器
	...
	if items.results.length
		...
		.list-pagination
			if items.totalPages > 1
				...
				.count 显示 #{utils.plural(items.total, '* ' + list.singular, '* ' + list.plural)}
