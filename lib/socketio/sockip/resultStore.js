...
//存储扫描结果到levelDB数据库
function resultStore(socket,key){
		...
		rs.on('end', function(){
			...
			resultRead(socket,key);
		});
};

//从leveldb数据库中读取扫描结果，并通过socket.volatile.emit发送到前端
function resultRead(socket,key){
		...
		cache.createReadStream()
			 .on('data', function(data) {
					var dkey = data.key,
						value = data.value;
					if(isContains(dkey, key)){
						 sendResult(dkey, value);
					}
				  })
