...
var client = net.connect({ port: 9099, host: '192.168.1.97' }, function() {
  ...
  var id = setInterval(function() {
    ...
    write({ key: '齿轮箱轴向振动', value: (Math.round(Math.random()*100)) / 30 });
    //write({ key: '温度', value: (Math.random() + x) / 50 });
    //write({ key: 'ohai', value: 1000 });
    write({ key: '主轴承径向振动', value: (Math.round(Math.random()*100+25)) / 20});
    //write({ key: '温度', value: (Math.random() + x) / 50 });
